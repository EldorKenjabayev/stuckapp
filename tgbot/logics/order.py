...
import random
from tgbot.models import Specialization, Specialist, Client
...
@sync_to_async
def is_specialist_busy(specialist_id: int):
    """Mutaxassisning bandligini tekshirish (Web + TG)"""
    from tgbot.models import Specialist
    from webtgbot.models import ChatSession
    
    specialist = Specialist.objects.filter(id=specialist_id).first()
    if not specialist:
        return True, "Специалист не найден."

    # 1. TG dagi bandlikni tekshirish
    if specialist.client:
        return True, "Специалист сейчас занят (в Telegram-боте) с другим пользователем."

    # 2. Web dagi bandlikni tekshirish
    web_session = ChatSession.objects.filter(specialist_id=specialist_id, is_active=True).first()
    if web_session:
        if web_session.is_expired:
            web_session.end_session()
        else:
            remain = web_session.remaining_seconds // 60
            if remain < 1: remain = 1
            return True, f"Специалист занят в веб-чате. Освободится примерно через {remain} мин."
...
def create_relation(client_id: int, specialist_id: int):
    try:
        ...
        if client is None or specialist is None:
            ...
            return False, "Клиент или специалист не найден."
        # Проверяем, есть ли у специалиста уже клиент в ТГ
        if specialist.client is not None:
            print(f"Specialist with ID {specialist_id} already has a client.")
            return False, "Специалист сейчас занят (в Telegram-боте) с другим пользователем."

        # Проверяем, есть ли у специалиста активная ВЕБ-сессия
        from webtgbot.models import ChatSession
        web_session = ChatSession.objects.filter(specialist=specialist, is_active=True).first()
        if web_session:
            if web_session.is_expired:
                web_session.end_session()
            else:
                print(f"Specialist with ID {specialist_id} is busy in web chat.")
                return False, "Специалист занят в веб-чате. Попробуйте позже."
        ...
        if client.specialist is not None:
            ...
            return False, "У вас уже есть активная сессия."

        # Проверяем, использовал ли клиент уже бесплатную сессию
        if client.has_used_free_session:
            return False, "Вы уже использовали свою бесплатную 30-минутную сессию."
        # Устанавливаем связь
        ...
        return False, "Ошибка при создании связи."
...
def end_active_relation(telegram_id: int):
    try:
        ...
        if client.specialist:
            ...
            client.specialist = None
            client.has_used_free_session = True # Помечаем, что сессия была использована
            client.save()
    ...
    try:
        ...
        if specialist.client:
            ...
            specialist.client.has_used_free_session = True # Помечаем для клиента
