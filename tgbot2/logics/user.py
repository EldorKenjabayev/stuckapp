...
from tgbot2.models import TelegramUser, Client, Specialist
...
def get_telegram_id_by_specialist_id(client_id: int) -> int or None:
    """Получение ID пользователя в Telegram по ID специалиста"""
    try:
        print(f"{client_id=}")
        specialist = Client.objects.get(id=client_id).telegram_user.telegram_id
        print(f"{specialist=}")
        return specialist
    except Specialist.DoesNotExist:
        return None
