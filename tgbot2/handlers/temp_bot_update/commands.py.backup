...
async def send_welcome(message: Message):
    ...
    kb.row(InlineKeyboardButton(text="🤳 Быстрый подбор брокера брокера", callback_data=f"tinder"))
...
async def groups(message: Message, command: CommandObject):
    if message.chat.type == "private":
        ...
        await message.answer("Список групп по города:", reply_markup=builder.as_markup())
